...
struct PiperJointHighSpeedFeedback3 : public can_data_plugins::CanDataBase
{
    ...
    double joint_3_actual_effort = 0.0;
    ...
    {
        // read joint 3 velocity from data[0] to data[1]
        int16_t joint_3_actual_velocity_raw = (int16_t(int8_t(data[0]) << 8) | uint8_t(data[1]));
        joint_3_actual_velocity = static_cast<double>(joint_3_actual_velocity_raw) / 1000.0; // Convert to rad/s
        // read joint 3 effort from data[2] to data[3]
        int16_t joint_3_actual_effort_raw = (int16_t(int8_t(data[2]) << 8) | uint8_t(data[3]));
        joint_3_actual_effort = static_cast<double>(joint_3_actual_effort_raw) * COEFFICIENT_1_to_3 / 1000.0; // Convert
        // RCLCPP_INFO(rclcpp::get_logger("PiperJointHighSpeedFeedback3"), "Read state for ID %d: Joint 3 Velocity: %.2f rad/s, Joint 3 Effort: %.2f N/m",
        //     id, joint_3_actual_velocity, joint_3_actual_effort);
    }
