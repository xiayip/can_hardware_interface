...
struct PiperJointHighSpeedFeedback6 : public can_data_plugins::CanDataBase
{
    std::string prefix;

    double COEFFICIENT_1_to_3 = 1.18125;
    double COEFFICIENT_4_to_6 = 0.95844;
    double joint_6_actual_velocity = 0.0;
    double joint_6_actual_effort = 0.0;
    ...
    void export_state_interface(std::vector<hardware_interface::StateInterface> &state_interfaces)
    {
        state_interfaces.emplace_back(
            hardware_interface::StateInterface(prefix + "joint6", hardware_interface::HW_IF_VELOCITY, &joint_6_actual_velocity));
        state_interfaces.emplace_back(
            hardware_interface::StateInterface(prefix + "joint6", hardware_interface::HW_IF_EFFORT, &joint_6_actual_effort));
    }
    ...
    {
        // read joint 6 velocity from data[0] to data[1]
        int16_t joint_6_actual_velocity_raw = (int16_t(int8_t(data[0]) << 8) | uint8_t(data[1]));
        joint_6_actual_velocity = static_cast<double>(joint_6_actual_velocity_raw) / 1000.0; // Convert to rad/s
        // read joint 6 effort from data[2] to data[3]
        int16_t joint_6_actual_effort_raw = (int16_t(int8_t(data[2]) << 8) | uint8_t(data[3]));
        joint_6_actual_effort = static_cast<double>(joint_6_actual_effort_raw) * COEFFICIENT_4_to_6 / 1000.0; // Convert
        // RCLCPP_INFO(rclcpp::get_logger("PiperJointHighSpeedFeedback6"), "Read state for ID %d: Joint 6 Velocity: %.2f rad/s, Joint 6 Effort: %.2f N/m",
        //     id, joint_6_actual_velocity, joint_6_actual_effort);
    }
