...
struct PiperJointMitCtrl6 : public can_data_plugins::CanDataBase
{
    double joint_6_target_position = std::numeric_limits<double>::quiet_NaN();
    double joint_6_target_velocity = std::numeric_limits<double>::quiet_NaN();
    double joint_6_target_kp = std::numeric_limits<double>::quiet_NaN();
    double joint_6_target_kd = std::numeric_limits<double>::quiet_NaN();
    double joint_6_target_torque = std::numeric_limits<double>::quiet_NaN();

    bool initialize(hardware_interface::ComponentInfo &joint)
    {
        joint_6_target_velocity = 0.0;
        joint_6_target_kp = 10.0;
        joint_6_target_kd = 0.8;
        joint_6_target_torque = 0.0;
        return true;
    }
    ...
    {
        RCLCPP_INFO(rclcpp::get_logger("PiperJointMitCtrl6"), "Exporting joint 6 command interfaces");
        command_interfaces.emplace_back(
            hardware_interface::CommandInterface("joint6", hardware_interface::HW_IF_POSITION, &joint_6_target_position));
        command_interfaces.emplace_back(
            hardware_interface::CommandInterface("joint6", hardware_interface::HW_IF_VELOCITY, &joint_6_target_velocity));
        command_interfaces.emplace_back(
            hardware_interface::CommandInterface("joint6", hardware_interface::HW_IF_EFFORT, &joint_6_target_torque));
    }
    ...
    {
        if (std::isnan(joint_6_target_position)) {
            return false;
        }
        unsigned int pos_tmp = FloatToUint(joint_6_target_position, -12.5, 12.5, 16); // TODO: read from joint parameters
        unsigned int vel_tmp = FloatToUint(joint_6_target_velocity, -45.0, 45.0, 12);
        unsigned int kp_tmp = FloatToUint(joint_6_target_kp, 0.0, 500.0, 12);
        unsigned int kd_tmp = FloatToUint(joint_6_target_kd, -5.0, 5.0, 12);
        unsigned int t_tmp = FloatToUint(joint_6_target_torque, -8.0, 8.0, 8);
        ...
        // RCLCPP_INFO(rclcpp::get_logger("PiperJointMitCtrl6"), "Write target for ID %03x: Joint 6 pos: %.2f, vel: %.2f, kp: %.2f, kd: %.2f, t: %.2f",
        //     id, joint_6_target_position, joint_6_target_velocity, joint_6_target_kp, joint_6_target_kd, joint_6_target_torque);
        return true;
    }
