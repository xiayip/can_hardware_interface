...
struct PiperJointHighSpeedFeedback4 : public can_data_plugins::CanDataBase
{
    ...
    double joint_4_actual_effort = 0.0;
    ...
    void read_state(const int /*id*/, const uint8_t data[8])
    {
        ...
        joint_4_actual_velocity = static_cast<double>(joint_4_actual_velocity_raw) / 1000.0; // Convert to rad/s
        // read joint 4 effort from data[2] to data[3]
        int16_t joint_4_actual_effort_raw = (int16_t(int8_t(data[2]) << 8) | uint8_t(data[3]));
        joint_4_actual_effort = static_cast<double>(joint_4_actual_effort_raw) * COEFFICIENT_4_to_6 / 1000.0; // Convert
        // RCLCPP_INFO(rclcpp::get_logger("PiperJointHighSpeedFeedback4"), "Read state for ID %d: Joint 4 Velocity: %.2f rad/s, Joint 4 Effort: %.2f N/m",
